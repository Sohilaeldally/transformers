...
class BilingualDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        torch.tensor([self.pad_token[0]]*enc_num_padding_tokens, dtype=torch.int64)
        ],
           dim=0
        )
        ...
        decoder_input = torch.cat(
            [
                ...
                torch.tensor([self.pad_token[0]]*dec_num_padding_tokens, dtype=torch.int64)
            ],
            dim=0
        )
        ...
        return {
            ...
            "encoder_mask"  : (encoder_input !=  self.pad_token[0]).unsqueeze(0).unsqueeze(0).int(), # (1,1,seq_len)
            "decoder_mask" : (decoder_input != self.pad_token[0]).unsqueeze(0).int() & causal_mask(decoder_input.size(0)) ,  # (1,seq_len) & (1,seq_len,seq_len)
            "label": label,  # (seq_len)
            "src_text": src_text,
            "tgt_text": tgt_text,
        }
